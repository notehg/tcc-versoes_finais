...
// Função para atualizar a média das avaliações na tabela files
function atualizarMediaAvaliacoes($conn, $livro_id) {
    $mediaAvaliacoes = calcularMediaAvaliacoes($conn, $livro_id);

    $sql = "UPDATE files SET media_avaliacoes = ? WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "di", $mediaAvaliacoes, $livro_id);
        if (mysqli_stmt_execute($stmt)) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['comentario'])) {
    ...
    if (isset($_POST['estrela']) && in_array($_POST['estrela'], ['1', '2', '3', '4', '5'])) {
        ...
        if (adicionarComentario($conn, $livro_id, $usuario_id, $comentario, $data_publicacao, $avaliacao)) {
            ...
            if (atualizarMediaAvaliacoes($conn, $livro_id)) {
                header("Location: det.php?id=$livro_id&success=1"); // Redireciona com parâmetro de sucesso
                exit();
            } else {
                $erro = "Erro ao atualizar a média de avaliações.";
            }
        } else {
            // Erro na inserção
